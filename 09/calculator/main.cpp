...
int main() {
    ...
    while ( true ) {
        ...
        if(cmd.str != "" && cmd.exit == true && pieces.size() == 1){
            cout<<"Thanks and see you later!"<<endl;
            break;
        } else if(cmd.str == "") {
            cout<<"Error: unknown command."<<endl;
        } else {
            if(pieces.size() > 3 || pieces.size() < 3){
                cout<<"Error: wrong number of parameters."<<endl;
            } else {
                double value1;
                double value2;
                if(string_to_double(pieces.at(1), value1) && string_to_double(pieces.at(2), value2)){
                    double result = cmd.action(value1, value2);
                    cout<<result<<endl;
                } else {
